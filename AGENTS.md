...
- `package-lock.json` is also committed but nothing installs from it; `pnpm-lock.yaml` is the only lockfile CI enforces. It drifts (has been seen out of sync with `package.json` for unrelated packages) - when patching a dependency vulnerability, fix `pnpm-lock.yaml` first via `pnpm audit`/targeted `pnpm.overrides` in `package.json`, then hand-align `package-lock.json`'s version/resolved/integrity entries for the same packages rather than trusting `npm install`/`npm audit fix` to regenerate it cleanly.
