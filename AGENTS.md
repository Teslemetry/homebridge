...
- Most logic changes still have no automated tests. Verify them by building (`npm run build`), linting (`npm run lint`), and, where useful, a throwaway smoke script that imports the compiled `dist/*.js`, mocks the `homebridge` `log`/`api` objects, and drives the relevant code path - see recent PR history for the pattern. A few committed cases do have real tests (e.g. `test/energy-battery.test.ts`, covering service construction/gating in `src/energy.ts`) using node's built-in test runner against fakes for the slice of the HAP API touched - no hap-nodejs/homebridge instance needed. Run them with `npm test` (compiles `src` + `test` via `tsconfig.test.json` into `dist-test`, then runs `node --test`).
